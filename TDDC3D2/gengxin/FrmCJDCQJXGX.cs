...
{
    public partial class FrmCJDCQJXGX : Form
    {
        ...
        private void btnDLTBGX_Click(object sender, EventArgs e)
        {
            ...
            IFeatureClass pGXGCFeatureclass = GPPolygonToLineByFeatureclass(filePath + "\\TDGX\\CJDCQGXGC");
            IFeatureClass pGXFeatureclass = GPPolygonToLineByFeatureclass(filePath + "\\TDGX\\CJDCQGX");
                                ...
                                else
                                {
                                    ...
                                    while ((pGXFeature = pGXFeaCursor.NextFeature()) != null)
                                    {
                                        if (GetIntersectLength(pGXGCFeature, pGXFeature) == true)
                                        {
                                            ...
                                            pFeaBuffer.set_Value(pCJDCQJX.FindField("YSDM"), "1000600200");
                                            pFeaBuffer.set_Value(pCJDCQJX.FindField("JXLX"), "660200");
                                            pFeaBuffer.set_Value(pCJDCQJX.FindField("JXXZ"), "600001");
                                            pFeaBuffer.set_Value(pCJDCQJX.FindField("GXSJ"), DateTime.ParseExact("20191231", "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture));
                        ...
                        if (tmp > 1)
                        {
                            ...
                            while ((pGXFeature = pGXFeatureCursor.NextFeature()) != null)
                            {
                                if (GetIntersectLength(tempFeature, pGXFeature) == true)
                                {
                                    ...
                                    pFeatureBuffer.set_Value(pCJDCQJX.FindField("YSDM"), "1000600200");
                                    pFeatureBuffer.set_Value(pCJDCQJX.FindField("JXLX"), "660200");
                                    pFeatureBuffer.set_Value(pCJDCQJX.FindField("JXXZ"), "600001");
                                    pFeatureBuffer.set_Value(pCJDCQJX.FindField("GXSJ"), DateTime.ParseExact("20191231", "yyyyMMdd", System.Globalization.CultureInfo.InvariantCulture));
        ...
        {
            IFeatureWorkspace pFeaWorkspace = currWs as IFeatureWorkspace;
            IFeatureClass pCJDCQJXClass = pFeaWorkspace.OpenFeatureClass("CJDCQJX");

            if (pCJDCQJXClass == null)
            {
                MessageBox.Show("村级调查区界线图层没有数据。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            UpdateStatus("正在生成标识码");
            IFeatureClass pGXClass = pFeaWorkspace.OpenFeatureClass("CJDCQJXGX");
            long maxBSM = RCIS.GISCommon.FeatureHelper.GetMaxStringNumberEveryOne(pCJDCQJXClass, "BSM");
