...
{
    public partial class FrmXZQSXPLGX : Form
    {
        public FrmXZQSXPLGX()
        {
            InitializeComponent();
        ...
        private void btnXZQCheck_Click(object sender, EventArgs e)
        {
            if (btnSelectBHXZQ.Text == "")
            {
                MessageBox.Show("请先选择统一时点更新行政区！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            string[] arr = { "bsm","xzqdm","xzqmc","mssm", "hdmc","dcmj","jsmj" };
            //string[] arr = { };
            List<string> fields = new List<string>();
            string shpFile = btnSelectBHXZQ.Text;
            ...
            else
            {
                ...
                MessageBox.Show("检查完毕，行政区数据结构符合要求！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void btnSelectBHXZQ_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            OpenFileDialog openFile = new OpenFileDialog();
            openFile.Filter = "shapefile(*.shp)|*.shp|All files(*.*)|*.*";
            if (openFile.ShowDialog() == DialogResult.OK)
            {
                btnSelectBHXZQ.Text = openFile.FileName;
                featureClass = null;
            }
        }
        ...
        private void btnEditBGXW_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请先进行行政区检查！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                sdFeatureclass = pFeatureWorkspace.OpenFeatureClass("XZQ");
            }
            catch
            {
            ...
            while (pFeature != null)
            {
                ...
                if (txFeture != null)
                {
                        ...
                        List<string> sdxzqdm = GetUniqueValuesByFeatureClass(sdFeatureclass,"XZQDM");
                        if (!sdxzqdm.Contains(pFeature.get_Value(pFeature.Fields.FindField("XZQDM")).ToString()))
                        {
            ...
            wait.Close();
            MessageBox.Show("行政区变更行为填写完毕！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
